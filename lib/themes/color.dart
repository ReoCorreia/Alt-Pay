import 'package:flutter/material.dart';

// qr theme color
const String qrColor = '#fff89224';

Color themeColorPrimary = const Color(0xFFf89224);
Color themeColorSecondary = const Color(0xFF464646); 
Color themeOrange = const Color(0xFFf89224);

// theme buttons
Color themeBtnGrey = const Color(0xFF464646);
Color themeBtnOrange = const Color(0xFFf89224);
Color lightGrey = const Color(0xFFD3D3D3);

//theme text
Color textWhite= const Color.fromARGB(255, 241, 241, 244);
Color textBlack = const Color.fromARGB(255, 0, 0, 0);
Color textRed = const Color.fromARGB(255, 255, 0, 0);
Color whitest = const Color.fromARGB(255, 255, 255, 255);

Color orange = Colors.orange;
Color purple = Colors.purple;
Color grey = Colors.grey;
Color black = const Color.fromARGB(255, 0, 0, 0);
Color lightGreen = const Color(0xFFCEFAD0);